...
const CampaignDetails = () => {
    ...
    const { doVote, address, getResult, endVote, getBallot } =
        useStateContext();
    ...
    useEffect(() => {
        const getSpecificBallot = async (id) => {
            try {
                ...
                console.log(res);
                const creationTimeBallot = parseInt(
                    res?.creationTime.toString()
                );
    ...
    const handleVote = async () => {
        try {
            ...
            if (error.message.includes("execution reverted:")) {
                ...
                while (nextIndex !== -1) {
                    const sentence = errorMessage.substring(
                        currentIndex,
                        nextIndex + 1
                    );
    ...
    const handleEndVote = async () => {
        try {
            setIsLoading(true);
            const res = await endVote(state.ballotId);
            setIsLoading(false);
        } catch (error) {
            setIsLoading(false);
    ...
    const id = getYouTubeVideoId(state.proposal);
